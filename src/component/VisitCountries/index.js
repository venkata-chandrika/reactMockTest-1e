...
class VisitCountries extends Component {
  ...
  visitCountry = id => {
    ...
    const visitedCountriesList = countriesList.map(eachList => {
      if (id === eachList.id) {
        return {...eachList, isVisited: !eachList.isVisited}
      }
      return eachList
    })
    this.setState({
      countriesList: visitedCountriesList,
    })
  }

  removeVisitedCountry = id => {
    const {countriesList} = this.state
    const removedCountriesList = countriesList.map(eachList => {
      if (id === eachList.id) {
        const removedItem = {
          ...eachList,
    ...
    this.setState({
      countriesList: removedCountriesList,
    })
  }
    ...
    return (
      <div className="bg-container">
        <div className="countries-container">
          ...
          <h1 className="country">Visited Countries</h1>
          <ul className="visited-countries">
            {updatedCountriesList.length > 0 ? (
              updatedCountriesList.map(eachList => (
                <VisitedCountry
                  key={eachList.id}
                  visitedCountry={eachList}
